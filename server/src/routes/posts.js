...
import { requireAccountAccessParam, requireTopicAccess } from '../middleware/accountAccess.js';

const router = Router();
router.post('/:topicId/generate-posts', requireTopicAccess, async (req, res, next) => {
  try {
...
router.get('/:accountId/posts', requireAccountAccessParam(), async (req, res, next) => {
